...
# Test
#
@test "Test case where local file mkdir fails" {
  ...
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -remotePath "$THE_TMP/foo/file" -CWS_outputdir $THE_TMP -maxRetry 2 -mkdirCmd "$THE_TMP/bin/command" $WF
  ...
  [ "${lines[1]}" == "detailed.error.message=Nonzero exit code (1) from $THE_TMP/bin/command -p $THE_TMP/data : error" ]
