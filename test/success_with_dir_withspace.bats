...
@test "Test success case with directory with space in name" {
  ...
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -remotePath "/foo hi" -CWS_outputdir $THE_TMP -maxRetry 2 -sleepCmd /bin/true -sshCmd "$THE_TMP/bin/command" -duCmd "$THE_TMP/bin/command" -rsyncCmd "$THE_TMP/bin/command" -mkdirCmd "$THE_TMP/bin/command" $WF
